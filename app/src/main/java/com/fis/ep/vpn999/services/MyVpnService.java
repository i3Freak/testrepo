...
public class MyVpnService extends VpnService {
    ...
    private void stopVpnConnection() {
        ...
        isRunning.set(false); // Set the flag to stop any loops relying on it
        if (VpnInterface != null) {
            try {
                VpnInterface.close();
                ...
                VpnInterface = null; // Ensure VpnInterface is set to null even if close() throws an exception
            }
        }
        onVpnConnectionDisconnected(); // Notify that the VPN is disconnected
    }
